...
RTC::ReturnCode_t WholeBodyMasterSlave::onExecute(RTC::UniqueId ec_id){
        ...
        for (auto tgt : tgt_names){
            if (m_masterTgtPosesIn[tgt]->isNew()){
                ...
                if(tgt == "lhand" || tgt == "rhand" || tgt == "lfloor" || tgt == "rfloor" ){
                    // nothing
                }else if(mode.now() == MODE_WBMS_ROT && (tgt == "larm" || tgt == "rarm")){
                    wbms->hp_wld_raw.stgt(tgt).abs.setRpy(hrp::to_Vector3(m_masterTgtPoses[tgt].data.orientation));
                }else if(mode.now() == MODE_WBMS_TRANS && (tgt == "larm" || tgt == "rarm")){
                    wbms->hp_wld_raw.stgt(tgt).abs.p = hrp::to_Vector3(m_masterTgtPoses[tgt].data.position);
                }else{
    ...
    ///// Button start
    static int button1_flag_count = 0, button2_flag_count = 0, button3_flag_count = 0, button4_flag_count = 0;
    ...
    if(m_masterTgtPoses["rhand"].data.position.z > 0 && m_masterTgtPoses["lhand"].data.position.z <= 0 ){ button3_flag_count++; }else{ button3_flag_count = 0; }
    if(m_masterTgtPoses["lhand"].data.position.z > 0 && m_masterTgtPoses["rhand"].data.position.z <= 0 ){ button4_flag_count++; }else{ button4_flag_count = 0; }
    if(button1_flag_count > 2.0 / m_dt){// keep pless both button for 2s to start
        ...
        if(mode.now() == MODE_WBMS || mode.now() == MODE_PAUSE || mode.now() == MODE_WBMS_ROT || mode.now() == MODE_WBMS_TRANS ){
    ...
    if(button2_flag_count > 2.0 / m_dt){// keep pless both button for 2s to start
        if(mode.now() == MODE_WBMS || mode.now() == MODE_WBMS_ROT || mode.now() == MODE_WBMS_TRANS ){
    ...
    if(button3_flag_count > 2.0 / m_dt){// keep pless right touchpad for 2s to rotation mode
        if(mode.now() == MODE_WBMS || mode.now() == MODE_WBMS_TRANS ){
            RTC_INFO_STREAM("change to rotation only mode by Button");
            changeWholeBodyMasterSlaveRotation();
        }
        if(mode.now() == MODE_WBMS_ROT ){
            RTC_INFO_STREAM("resumeWholeBodyMasterSlave by Button");
            resumeWholeBodyMasterSlave();
        }
        button3_flag_count = 0;
    }
    if(button4_flag_count > 2.0 / m_dt){// keep pless right touchpad for 2s to rotation mode
        if(mode.now() == MODE_WBMS || mode.now() == MODE_WBMS_ROT ){
            RTC_INFO_STREAM("change to translation only mode by Button");
            changeWholeBodyMasterSlaveTranslation();
        }
        if(mode.now() == MODE_WBMS_TRANS ){
            RTC_INFO_STREAM("resumeWholeBodyMasterSlave by Button");
            resumeWholeBodyMasterSlave();
        }
        button4_flag_count = 0;
    }
    ...
    if (mode.isRunning()) {
        ...
        }else if(mode.isCurrentInitialize()){
            wbms->initializeRequest(fik->m_robot, ee_ikc_map);
            idsb.setInitState(fik->m_robot, m_dt);
        }
...
void WholeBodyMasterSlave::processTransition(){
    switch(mode.now()){
        ...
        case MODE_SYNC_TO_WBMS_ROT:
            mode.setNextMode(MODE_WBMS_ROT);
            break;
...
bool WholeBodyMasterSlave::resumeWholeBodyMasterSlave(){
    if(mode.now() == MODE_PAUSE || mode.now() == MODE_WBMS_ROT || mode.now() == MODE_WBMS_TRANS ){
...
bool WholeBodyMasterSlave::stopWholeBodyMasterSlave(){
    if(mode.now() == MODE_WBMS || mode.now() == MODE_PAUSE || mode.now() == MODE_WBMS_ROT || mode.now() == MODE_WBMS_ROT ){
...
bool WholeBodyMasterSlave::changeWholeBodyMasterSlaveRotation(){
    if(mode.now() == MODE_WBMS || mode.now() == MODE_WBMS_TRANS ){
        RTC_INFO_STREAM("changeWholeBodyMasterSlaveRotation");
        mode.setNextMode(MODE_SYNC_TO_WBMS_ROT);
        return true;
    }else{
        RTC_WARN_STREAM("Invalid context to changeWholeBodyMasterSlaveRotation");
        return false;
    }
}


bool WholeBodyMasterSlave::changeWholeBodyMasterSlaveTranslation(){
    if(mode.now() == MODE_WBMS || mode.now() == MODE_WBMS_ROT ){
        RTC_INFO_STREAM("changeWholeBodyMasterSlaveTranslation");
        mode.setNextMode(MODE_SYNC_TO_WBMS_TRANS);
        return true;
    }else{
        RTC_WARN_STREAM("Invalid context to changeWholeBodyMasterSlaveTranslation");
        return false;
    }
}
