...
enum mode_enum{ MODE_IDLE, MODE_SYNC_TO_WBMS, MODE_WBMS, MODE_PAUSE, MODE_SYNC_TO_IDLE, MODE_WBMS_ROT, MODE_SYNC_TO_WBMS_ROT, MODE_WBMS_TRANS, MODE_SYNC_TO_WBMS_TRANS};
...
class ControlMode{
    ...
    public:
        ...
        bool setNextMode(const mode_enum _request){
            switch(_request){
                case MODE_SYNC_TO_WBMS:
                    if(current == MODE_IDLE || current == MODE_PAUSE || current == MODE_WBMS_ROT || current == MODE_WBMS_TRANS ){ next = MODE_SYNC_TO_WBMS; return true; }else{ return false; }
                case MODE_WBMS:
                    if(current == MODE_SYNC_TO_WBMS){ next = MODE_WBMS; return true; }else{ return false; }
                case MODE_PAUSE:
                    if(current == MODE_WBMS || current == MODE_WBMS_ROT || current == MODE_WBMS_TRANS ){ next = MODE_PAUSE; return true; }else{ return false; }
                case MODE_SYNC_TO_IDLE:
                    if(current == MODE_WBMS || current == MODE_PAUSE || current == MODE_WBMS_ROT || current == MODE_WBMS_TRANS ){ next = MODE_SYNC_TO_IDLE; return true; }else{ return false; }
                case MODE_IDLE:
                    if(current == MODE_SYNC_TO_IDLE ){ next = MODE_IDLE; return true; }else{ return false; }
                case MODE_SYNC_TO_WBMS_ROT:
                    if(current == MODE_WBMS || current == MODE_WBMS_TRANS ){ next = MODE_SYNC_TO_WBMS_ROT; return true; }else{ return false; }
                case MODE_WBMS_ROT:
                    if(current == MODE_SYNC_TO_WBMS_ROT ){ next = MODE_WBMS_ROT; return true; }else{ return false; }
                case MODE_SYNC_TO_WBMS_TRANS:
                    if(current == MODE_WBMS || current == MODE_WBMS_ROT ){ next = MODE_SYNC_TO_WBMS_TRANS; return true; }else{ return false; }
                case MODE_WBMS_TRANS:
                    if(current == MODE_SYNC_TO_WBMS_TRANS ){ next = MODE_WBMS_TRANS; return true; }else{ return false; }
                default:
                    return false;
            }
        }
        ...
        bool isRunning(){ return (current==MODE_SYNC_TO_WBMS) || (current==MODE_WBMS) || (current==MODE_PAUSE) || (current==MODE_SYNC_TO_IDLE) || (current==MODE_WBMS_ROT) || (current==MODE_SYNC_TO_WBMS_ROT) || (current==MODE_WBMS_TRANS) || (current==MODE_SYNC_TO_WBMS_TRANS);}
        bool isInitialize(){ return (previous==MODE_IDLE) && (current==MODE_SYNC_TO_WBMS) ;}
        bool isCurrentInitialize() { return ((previous==MODE_SYNC_TO_WBMS) && (current==MODE_WBMS)) || ((previous==MODE_SYNC_TO_WBMS_ROT) && (current==MODE_WBMS_ROT)) || ((previous==MODE_SYNC_TO_WBMS_TRANS) && (current==MODE_WBMS_TRANS)) ;} // start時に2回初期化してしまう TODO yoshioka
};
...
class WholeBodyMasterSlave : public RTC::DataFlowComponentBase{
    public:
        ...
        bool changeWholeBodyMasterSlaveRotation();
        bool changeWholeBodyMasterSlaveTranslation();
        bool changeWholeBodyMasterSlaveDefault();
